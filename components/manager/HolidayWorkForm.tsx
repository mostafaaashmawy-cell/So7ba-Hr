      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
        <div>
          ...
          <select
            ...
            required
            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500 focus:outline-none focus:border-blue-500 cursor-pointer"
          >
        ...
        <div>
          ...
          <input
            ...
            required
            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500 focus:outline-none focus:border-blue-500 font-sans"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            required
            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500 focus:outline-none focus:border-blue-500"
          />
        </div>
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-100 dark:bg-slate-800 text-slate-950 dark:text-slate-200 uppercase text-[10px] tracking-wider font-extrabold">
                <tr>
                  ...
                  <th className="px-4 py-3.5 dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-800">{isRtl ? 'تاريخ العمل' : 'Working Date'}</th>
                  <th className="px-4 py-3.5 dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-800">{isRtl ? 'الملاحظات' : 'Notes'}</th>
                  <th className="px-4 py-3.5 text-right dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-800">{isRtl ? 'الإجراءات' : 'Actions'}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
                {records.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 dark:border-slate-800/60 border-b last:border-none transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-right dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-800">
                      <button
                        type="button"
