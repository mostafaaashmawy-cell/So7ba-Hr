...
export default function EmployeePerformanceTable({ employees }: EmployeePerformanceTableProps) {
  ...
  const getPerformanceBadge = (idx: number, role: string) => {
    if (role === 'super_admin' || idx % 3 === 0) {
      return (
        <span className="px-3 py-1 rounded-full text-[11px] font-bold bg-blue-50 text-blue-600 border border-blue-200 uppercase tracking-wide font-sans">
          Excellent
        </span>
      );
    } else if (idx % 3 === 1) {
      return (
        <span className="px-3 py-1 rounded-full text-[11px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-200 uppercase tracking-wide font-sans">
          Good
        </span>
      );
    } else {
      return (
        <span className="px-3 py-1 rounded-full text-[11px] font-bold bg-amber-50 text-amber-600 border border-amber-200 uppercase tracking-wide font-sans">
          Average
        </span>
      );
    }
  ...
  return (
    <div className="cleariq-card p-6 cleariq-card-hover space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-base font-bold text-slate-900">Employee Performance</h3>
