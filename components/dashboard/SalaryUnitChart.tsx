...
export default function SalaryUnitChart({
  ...
  const displayTitle = title || (isRtl ? 'إجمالي الأجور والنشاط حسب الوحدة' : 'Total Salary / Activity by Unit');
  const displaySubtitle = subtitle || (isRtl ? 'مقارنة شهرية بين أنشطة العمليات والمبيعات' : 'Monthly comparison between Operations & Sales activities');

  const monthsData = [
    { month: isRtl ? 'يناير' : 'Jan', ops: 45, sales: 30 },
    { month: isRtl ? 'فبراير' : 'Feb', ops: 65, sales: 48 },
    { month: isRtl ? 'مارس' : 'Mar', ops: 38, sales: 25 },
    { month: isRtl ? 'أبريل' : 'Apr', ops: 78, sales: 55 },
    { month: isRtl ? 'مايو' : 'May', ops: 52, sales: 40 },
    { month: isRtl ? 'يونيو' : 'Jun', ops: 88, sales: 62 },
    { month: isRtl ? 'يوليو' : 'Jul', ops: 70, sales: 50 },
    { month: isRtl ? 'أغسطس' : 'Aug', ops: 92, sales: 75 },
    { month: isRtl ? 'سبتمبر' : 'Sep', ops: 60, sales: 45 },
    { month: isRtl ? 'أكتوبر' : 'Oct', ops: 82, sales: 68 },
    { month: isRtl ? 'نوفمبر' : 'Nov', ops: 74, sales: 58 },
    { month: isRtl ? 'ديسمبر' : 'Dec', ops: 95, sales: 80 },
  ];
  ...
  return (
    <div className="cleariq-card p-6 cleariq-card-hover space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h3 className="text-base font-bold text-slate-950 dark:text-white">{displayTitle}</h3>
        ...
        {monthsData.map((d, idx) => {
          ...
          return (
            ...
            >
              {/* Tooltip on hover */}
              {isHovered && (
                <div className="absolute -top-10 z-20 bg-slate-900 dark:bg-slate-800 text-white text-[10px] font-bold px-2 py-1 rounded-lg shadow-lg whitespace-nowrap animate-in fade-in border border-slate-700 font-sans">
                  <span>{isRtl ? 'العمليات' : 'Ops'}: {d.ops}k</span> • <span>{isRtl ? 'المبيعات' : 'Sales'}: {d.sales}k</span>
                </div>
              )}
              ...
              {/* Month Label */}
              <span
                className={`text-[10px] sm:text-[11px] font-semibold mt-2 transition-colors truncate max-w-full text-center ${
