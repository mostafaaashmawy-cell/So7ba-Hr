        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 cleariq-card px-4 py-2.5">
            ...
            <div>
              ...
              <span className="text-base font-extrabold text-slate-900 dark:text-slate-100 font-sans">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {isPrivileged && (
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500 cursor-pointer dark:placeholder:text-slate-500"
                  >
                    {teamMembers.map((m) => (
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500 font-sans dark:placeholder:text-slate-500"
                />
              </div>
          ...
          <div className="lg:col-span-2 cleariq-card p-6 cleariq-card-hover space-y-5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800 text-slate-400 text-[11px] font-semibold uppercase tracking-wider">
                    <th className="pb-3 pl-2 dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-800">{isRtl ? 'الموظف' : 'Employee'}</th>
                    <th className="pb-3 dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-800">{isRtl ? 'المبلغ' : 'Amount'}</th>
                    <th className="pb-3 dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-800">{isRtl ? 'التاريخ' : 'Date'}</th>
                    <th className="pb-3 text-center dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-800">{isRtl ? 'الحالة' : 'Status'}</th>
                    <th className="pb-3 pr-2 text-right dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-800">{isRtl ? 'الإجراءات' : 'Actions'}</th>
                  </tr>
                </thead>
                          ...
                          {isPrivileged && item.status === 'pending' && (
                            <>
                              <button
                                ...
                                className="p-1 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-400 dark:border-emerald-800"
                                title="Approve"
                              >
                              ...
                              <button
                                ...
                                className="p-1 rounded-lg bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200 dark:bg-rose-950/40 dark:text-rose-400 dark:border-rose-800"
                                title="Reject"
                              >
